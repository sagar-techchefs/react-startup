...
class ImangeSearch extends React.Component {
    ...
    getImages = async (searchValue) => {
        ...
        this.setState({ images: response.data.results, isLoaderActive: false });
        // urls.regular
        // console.log(this.state.images);
    ...
    render() {
        return (
            <div>
                {this.state.isLoaderActive ? (<Spinner message="Please wait fetching images" />) : null}
                <SearchBar onSearch={this.onSearch} onSubmit={this.onSubmit} />
